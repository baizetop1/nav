...
- 首页提供“热榜与动态”区域：展示社会热榜和 `baizetop1` 的 GitHub 公开动态，支持收起、手动刷新、过期提醒和本机缓存；工作模式会自动压缩为每栏 5 条。
...
- GitHub Actions 每小时生成 `public/hot-feed.json` 并重新部署；社会榜单优先读取今日头条、失败时切换百度并保留上一次有效数据，GitHub 动态使用官方公开 Events API。
