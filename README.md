...
- 首页提供“热榜聚合”区域：左侧展示社会热榜，右侧展示 GitHub 今日热门仓库及语言、总 Star、今日新增 Star；支持收起、手动刷新、过期提醒和本机缓存，工作模式会自动压缩为每栏 5 条。
...
- GitHub Actions 每小时生成 `public/hot-feed.json` 并重新部署；社会榜单优先读取今日头条、失败时切换百度，开发热榜读取 GitHub Trending；任一来源失败时都会保留上一次有效数据。
