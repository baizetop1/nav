import {rotationCalendar} from './rotations.js?v=0.26.0';
import {requireRule,count,journal} from './utils.js?v=0.26.0';
...
{id:'daily',name:'每日勤勉荐书',period:cal.date,cost:0,reward:{items:{recruit_order:1,recruit_shard:2}},reason:!c?'先建立寨子':s.daily.counters.supply_daily?'今日已领取':!(s.daily.counters.campWork>=1&&s.daily.counters.battleWin>=1)?'今日经营一次并取得一场胜利':''},
...
export function claimSupply(s,d,id){const q=supplyOffers(s).find(q=>q.id===id);requireRule(q&&!q.reason,q?.reason||'没有这项悬赏');s.player.merit-=q.cost;if(id==='daily')count(s,'supply_daily');else{ s.camp.supply??={};s.camp.supply[id]=q.period;}grant(s,q.reward,d);journal(s,'【寨中悬赏】'+q.name+'已领取'+(q.cost?'，功勋 -'+q.cost:'')+'。');}
