...
import { experienceResult } from './progression.js?v=0.28.0';
import { applySets } from './equipment-sets.js?v=0.28.0';
import { qualityOf, QUALITIES } from './quality.js?v=0.28.0';
import { bounded, count, journal, pick, random, requireRule, weighted } from './utils.js?v=0.28.0';
import { heroRank } from './map.js?v=0.28.0';
import { unlockReason, skillLevel, trainedSkill } from './growth.js?v=0.28.0';
...
export function attributes(state, id, data, level = state.heroes[id].level, withEquipment = true) {
  ...
  if(withEquipment)applySets(state,id,result);
  ...
  if(withEquipment){const q=QUALITIES[qualityOf(state.heroes[id])];for(const key of Object.keys(result))result[key]=Math.round(result[key]*(key==='speed'?q.speed:q.power));}
  return result;
}
