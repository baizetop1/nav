import {initializeExpansion} from './expansion-combat.js?v=0.31.0';
import {settleVeterans} from './veterans.js?v=0.31.0';
import {recruitPrice} from './realm-buildings.js?v=0.31.0';
import {initializeRealmBattle} from './realm-combat.js?v=0.31.0';
import { reservedTroops } from './squads.js?v=0.31.0';
import { FIELD_RULES, casualtyQuote, initializeSignatures } from './fieldcraft.js?v=0.31.0';
import { claimSupply } from './supplies.js?v=0.31.0';
import { barracksCapacity, deployedTroops, troopShares, troopPower } from './logistics.js?v=0.31.0';
import { isExternal, externalInvitation } from './roster.js?v=0.31.0';
import { openAffair } from './affairs.js?v=0.31.0';
import { initializeStrategy } from './strategy.js?v=0.31.0';
import { initializeDoctrine } from './doctrines.js?v=0.31.0';
import { CORPS, corpsRank } from './development.js?v=0.31.0';
import { ARMS } from './martial.js?v=0.31.0';
import { hireHelper, helperBonus, validateHelpers } from './helpers.js?v=0.31.0';
import { finishDuty, developmentAction, raidBonus, RAID_INTEL, searchEquipment } from './camp-development.js?v=0.31.0';
import { hasOwn, requireRule, journal, count } from './utils.js?v=0.31.0';
import { ownHero } from './hero.js?v=0.31.0';
...
export const freshCamp=()=>({version:1,day:1,wood:110,food:100,troops:0,wounded:0,buildings:{hall:1,farm:0,lumber:0,barracks:0,clinic:0,market:0},mode:'army',tactic:'balanced',deployment:100,work:0,sorties:0});
...
export function campAction(s,d,a){
  ...
  if(a.type==='campSupply'){claimSupply(s,d,a.id);
  }else if(a.type==='campHireHelper'){hireHelper(s,a.id);
  }else if(a.type==='campSteward'||a.type==='campClaim'){developmentAction(s,d,a);
  }else if(a.type==='campBuild'){
    requireRule(hasOwn(BUILDINGS,a.id),'没有这项设施。');const lv=c.buildings[a.id],q=buildingQuote(c,a.id);requireRule(lv<5,'设施已满级。');requireRule(a.id==='hall'||lv<c.buildings.hall,'先扩建聚义厅。');
...
export function validateCamp(s,check){
  ...
  if(s.battle?.expedition){const e=s.battle.expedition;if(e.fieldRules!==undefined)check(e.fieldRules===FIELD_RULES,'行军规则版本');if(e.arm!==undefined)check(['infantry','ranged','cavalry'].includes(e.arm),'出征兵种');check(num(e.troops,1000)&&e.troops<=c.troops&&hasOwn(TACTICS,e.tactic)&&!s.battle.guest,'出征兵力');}
}
