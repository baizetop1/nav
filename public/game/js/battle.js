...
function hit(state,u,skill,data){
  ...
  if(effect.kind==='heal'){
    ...
    reportHealing(b,u,heal,target);target.hp+=heal;log(b,`${u.name}施展【${skill.name}】，照应${target.name}，回复 ${heal} 点气血。`);return;
  }
...
export function setBattleSkillMode(state,mode){
  ...
  requireRule(b.context.type!=='lesson','演武使用手动技能，练习亲自判断时机。');
  ...
  while(!b.outcome){
    ...
    for(const u of units){
      for(const s of u.statuses)if(s.nextTickAt===next&&alive(u)){
        const loss=journeyAbsorb(b,u,s.id==='bleeding'?Math.max(1,Math.round(u.maxHp*.02)):Math.round(s.value));reportOtherDamage(b,u,loss,'dot');u.hp=Math.max(0,u.hp-loss);
...
export function useBattleItem(state,data,id){
  ...
  const b=state.battle,item=data.by.items[id];state.inventory[id]--;if(b.metrics)b.metrics.medicineUses=(b.metrics.medicineUses||0)+1;
...
export function retreatBattle(state){const b=state.battle;requireRule(b&&!b.outcome,'当前没有可撤出的战斗。');b.outcome='retreat';if(b.metrics)b.metrics.reason='manual';log(b,b.context.type==='lesson'?'演武已停止，没有消耗物资或出征次数。':'你下令退回安全处。此战未取胜，已消耗的药物与历练次数不会返还。');}
