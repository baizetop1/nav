import { gameSnapshot, slotId, slotName } from './portable.js?v=0.20.0';
...
export class CloudClient {
  ...
  async upload(id,revision,state,name){const elite=state.battle?.context.type==='elite'||state.lastBattle?.context.type==='elite',reports=state.lastBattle||state.battle?.metrics,affairs=state.affairs,strategy=state.strategy,commands=state.commandVersion===1,frontier=state.frontier||state.battle?.frontierRules,advanced=state.development||state.battle?.martial===2,rotations=state.campaign||state.battle?.martial||state.camp?.arm;if(elite||reports||affairs||strategy||commands||frontier||advanced||rotations){const version=await this.request('/v1/game-version');if(elite&&version.elite!==1)throw new Error('请先更新云存档服务至 0.20.0，再上传精英战局与复盘。本机进度已保留。');if(commands&&version.commands!==1)throw new Error('请先更新云存档服务至 0.12.0，再上传军令、十连与装备锁定进度。本机进度已保留。');if(frontier&&version.frontier!==1)throw new Error('请先更新云存档服务至 0.11.0，再上传领地生产与新战法进度。本机进度已保留。');if(advanced&&version.development!==1)throw new Error('请先更新云存档服务至 0.10.0，再上传专属兵种与阵容进度。本机进度已保留。');if(rotations&&version.rotations!==1)throw new Error('请先更新云存档服务至 0.9.0，再上传轮换历练与兵种进度。本机进度已保留。');if(affairs&&version.management!==1)throw new Error('请先更新云存档服务至 0.14.0，再上传寨事和外派进度。本机进度已保留。');if(strategy&&version.strategy!==1)throw new Error('请先更新云存档服务至 0.13.0，再上传部队专精和副本机制进度。本机存档已保留。');if(reports&&version.reports!==1)throw new Error('请先更新云存档服务至 0.16.0，再上传战斗复盘记录。本机进度已保留。');}id=slotId(id);return this.request(`/v1/slots/${id}`,{id,method:'PUT',revision,body:{state:gameSnapshot(state,this.data),name}});}
