...
export const collections=['heroes','skills','items','equipments','enemies','maps','stories','schemes','dungeons','rewards','events','quests'];
...
export function prepareData(raw) {
  ...
  const condition=c=>{if(!c)return;for(const [key,value] of Object.entries(c)){
    ...
    else requireRule(['flag','notFlag','count','status','time','liangshanLevel','ownedCount','heroLevel','equipmentCount','unvisited','feature'].includes(key),'未知条件：'+key);
  }};
  ...
  for(const story of data.stories){ref('maps',story.map);condition(story.condition);requireRule(story.steps[story.start],'剧情起点不存在。');for(const step of Object.values(story.steps)){ref('maps',step.map);condition(step.condition);requireRule(step.text&&step.choices.length,'剧情缺少文本或选项。');for(const c of step.choices){effects(c.effects);if(!c.finish)requireRule(story.steps[c.next],'剧情后续步骤不存在。');if(c.goMap)ref('maps',c.goMap);if(c.battle){c.battle.enemies.forEach(id=>ref('enemies',id));if(c.battle.guest)ref('heroes',c.battle.guest.id);}}}}
  for(const d of data.dungeons){ref('maps',d.map);ref('rewards',d.reward);d.enemy.forEach(id=>ref('enemies',id));condition(d.condition);requireRule(d.level>=1&&d.level<=40&&d.limit===3&&numeric(d.cost,1),'历练数值无效。');}
  ...
  requireRule(data.config.version===1&&data.config.balance.ordinaryRates.reduce((s,e)=>s+e.weight,0)===100,'配置版本或招贤权重无效。');
  return data;
}
