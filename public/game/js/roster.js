import { hasOwn, requireRule } from './utils.js?v=0.31.0';
...
export const ordinaryHeroes=d=>d.heroes.filter(h=>!isExternal(h)||isWanderer(h));
export const canonicalHeroes=d=>d.heroes.filter(h=>!isExternal(h));
export function externalInvitation(s,h){if(isWanderer(h)){const cost={silver:h.star*120,items:{}};return {cost,reason:s.heroes[h.id]?.status==='owned'?'已入寨':!s.camp||s.camp.buildings.hall<1||s.camp.work+s.camp.sorties<h.star?'聚义厅 1 级，经营与胜利出征合计须达 '+h.star+' 次':s.player.silver<cost.silver?'碎银不足':''};}const c=h.obtain,price={silver:c.silver,items:{[c.token]:c.count}};const reason=s.heroes[h.id]?.status==='owned'?'已入寨':!['known','available'].includes(s.heroes[h.id]?.status)||!meets(s,c.condition)?'先完成这位人物的外传相识与委托':!s.camp||s.camp.buildings.hall<c.hall?'聚义厅须达到 '+c.hall+' 级':s.player.silver<c.silver?'碎银不足':(s.inventory[c.token]||0)<c.count?'人物信物不足':'';return {cost:price,reason};}
