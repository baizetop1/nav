...
function heroCard(s,d,h){
  ...
  return `<article class="card"><h2>${esc(h.title)} · ${esc(h.name)}<span class="badge">${statusLabels[v.status]}</span></h2>
...
export function render({state:s,data:d,view,status='',error='',locked=false,notice='',recruitTarget='',entered=true,battlePaused=false,battlePauseReason='',saveBox={},activity=null}){
  ...
  if(screen==='welcome')content=`<section class="intro opening"><p class="kicker">第一卷 · 郓城风起</p><h1>江湖，由此入卷。</h1><div class="intro-line" aria-hidden="true"></div><div class="opening-lines">${d.config.opening.map(t=>`<p>${esc(t)}</p>`).join('')}</div><div class="opening-actions">${btn('踏入郓城县',{type:'ui_start'},'primary')}${nav('接续旧卷','save')}</div><p class="note">一页文字，一片江湖。入城后开始本机保存。</p></section>`;
