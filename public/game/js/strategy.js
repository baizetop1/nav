import { reportHealing, reportOtherDamage } from './debrief.js?v=0.24.0';
import { HERO_SPECIALTIES, DRILLS, STYLES, TERRAIN_NAMES, MECHANICS, activeBonds, battleTerrain } from './strategy-data.js?v=0.24.0';
...
export function strategyFollowup(b,u,target,api){
 ...
 if(p.style==='medic'&&low){const n=Math.min(low.maxHp-low.hp,Math.round(low.maxHp*.03));reportHealing(b,u,n);low.hp+=n;if(n)api.log(b,`【援护】${u.name}为${low.name}回复 ${n} 气血。`);}
