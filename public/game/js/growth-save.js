...
export function validateGrowthBattle(b,data,check){
  ...
  for(const u of b.enemy){
    ...
    if(u.boss!==undefined){const boss=u.boss,kind=b.journey?.rules===2&&b.journey.kind==='boss'&&u===b.enemy[0]?'journey_'+b.journey.region:({tiger_king:'tiger',bandit_chief:'chief',road_raider:'raider',v5_luan_tingyu:'chief',v5_zhu_biao:'raider',v6_gaolian:'chief',v6_rider:'raider'})[u.model];
      check((['dungeon','rotation','frontier','realm','lesson'].includes(b.context?.type)||isChapterBattle(b))&&object(boss)&&kind&&boss.kind===kind&&integer(boss.phase,0,1)&&integer(boss.readyAt,0,190000)&&integer(boss.pendingAt,0,190000),'首领快照');
