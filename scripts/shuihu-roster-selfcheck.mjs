...
assert.equal(data.heroes.filter(h=>h.group!=='external').length,108);assert.equal(data.heroes.filter(h=>h.group==='tiangang').length,36);assert.equal(data.heroes.filter(h=>h.group==='disha').length,72);
assert.equal(data.heroes.filter(h=>h.group!=='external').at(-1).name,'段景住');assert.equal(data.heroes[1].name,'卢俊义');assert.equal(new Set(data.heroes.map(h=>h.name)).size,data.heroes.length);assert.equal(data.by.heroes.chaogai.group,'external');
...
const pages=Array.from({length:9},(_,page)=>rosterSelection(s,data,{page,group:'canonical'}).visible.map(h=>h.id)).flat();assert.equal(new Set(pages).size,108);
...
const migrated=parseSave(JSON.stringify(legacy),data);assert.equal(migrated.rosterVersion,data.config.rosterVersion);assert.equal(Object.keys(migrated.heroes).length,data.heroes.length);assert.deepEqual(migrated.heroes.wusong,legacy.heroes.wusong);assert.equal(migrated.heroes.lujunyi.status,'unknown');assert.deepEqual(parseSave(JSON.stringify(migrated),data),migrated);
...
const normalized=parseSave(JSON.stringify(cachedRuntime),data);assert.equal(normalized.rosterVersion,data.config.rosterVersion);assert.equal(normalized.heroes.guansheng.quality,2);
