...
let joined=structuredClone(beforeTen);for(const h of ordinaryHeroes(d))joined.heroes[h.id].status='known';joined=draw(joined);assert.equal(joined.recruit.support.dry,0);assert.equal(joined.recruit.support.tickets,0);assert.equal(joined.recruit.support.points,9);
