...
let production=act(fixture(),'frontierStart');const base=stationYield(production,d,'workshop');production=act(production,'frontierAssign',{id:'workshop',worker:'hero:tanglong'});assert.equal(stationYield(production,d,'workshop'),base+4);production=act(production,'refresh',{},production.clock+CYCLE);assert.equal(production.frontier.stations.workshop.bank,5);production=act(production,'team',{ids:['tanglong']});production=act(production,'campRaid',{id:'woods'});assert.equal(stationYield(production,d,'workshop'),base);
...
console.log('Strategy: all 108 specialties, 16 reachable bonds, paid drill tradeoffs, production/absence, six real style effects, all 7 objective timers, escort failure/no rewards, tide mitigation, finite reinforcement, time slicing, old battles, corruption and cloud guards passed.');
