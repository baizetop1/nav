...
for(const [kind,count] of Object.entries({heroes:112,maps:79,items:482,equipments:20,skills:454,events:24,dungeons:9}))assert.equal(data[kind].length,count);
...
assert.equal(state.progress.flags.seven_stars,true);assert.equal(state.heroes.wuyong.status,'known');assert.equal(Object.keys(state.heroes).length,data.heroes.length);
