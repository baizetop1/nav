...
for(const d of data.dungeons.filter(d=>d.kind!=='scheme'))for(const [name,ids] of Object.entries(teams)){
 ...
 results.push({dungeon:d.id,team:name,wins:victories,total:30,seconds:Math.round(totalTime/30000)});
}
console.table(results);
// At matching recommended levels and rank-3 bonds, multiple roles must be viable.
for(const d of data.dungeons.filter(d=>d.kind!=='scheme'))assert.ok(results.filter(r=>r.dungeon===d.id&&r.wins>=18).length>=3,d.name+' must allow at least three viable formations');
for(const name of Object.keys(teams))assert.ok(results.some(r=>r.team===name&&r.wins>=18),name+' must have a useful matchup');
console.log('Shuihu balance: 900 seeded battles, all 14 heroes across five formations, six combat dungeons, no automatic items, bounded fight clocks and save validation passed.');
