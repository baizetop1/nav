...
const html=render({state:fixture(),data,view:'map',mapSection:'explore'});for(const text of ['副本寻骑','坐骑契','景阳冈历练','风雪山神庙历练','待副本获得'])assert.ok(html.includes(text));
const earned=win(fixture(),'jingyanggang');earned.location='stable';assert.ok(render({state:earned,data,view:'map',mapSection:'explore'}).includes('可凭契领骑'));earned.location='jingyang';assert.ok(render({state:earned,data,view:'map',mapSection:'explore'}).includes('坐骑契掉落'));assert.ok(render({state:earned,data,view:'heroes',pageSections:{heroes:'training'}}).includes('领骑仅消耗坐骑契 1 张'));
