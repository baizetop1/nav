...
const ui=render({state:trained,data,view:'heroes',pageSections:{heroes:'skills'}});for(const h of data.heroes){const detail=render({state:trained,data,view:'heroes',pageSections:{heroes:'skills'},roster:{selected:h.id}});for(const id of h.skills)assert.ok(detail.includes(data.by.skills[id].name));}for(const text of ['skillUpgrade','mountFeed','mountRank','martialDrill','材料去哪里找','亲密'])assert.ok(ui.includes(text));
