...
- 首页提供可切换的技术情报（综合、AI、安全、开发）与 GitHub Trending 热门仓库；GitHub Actions 每小时在服务端聚合官方 API/RSS 为 `public/hot-feed.json`，前端只读取同源静态报告，不暴露 Token，并在更新失败时回退已部署的上次有效数据与本机缓存。
