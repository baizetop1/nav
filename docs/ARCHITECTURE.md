...
- 分类拖拽排序和跨分类拖动网站。
- 自由网格坐标和卡片尺寸调整。
- GitHub Actions 部署状态轮询和远端内容合并界面。
...
### 阶段 4：拖拽与搜索增强（基础能力已完成）

- 已引入 dnd-kit 支持全局网站排序；跨分类拖动和自由网格仍待评估。
...
- GitHub 仓库中的 JSON 是发布数据的唯一事实来源；LocalStorage 只保存草稿和偏好。
- 前端构建变量不是秘密，不能用于安全认证或保存 GitHub Token。
- 第一版优先完成稳定排序，不提前设计自由桌面坐标。
- 标签先由网站数据聚合，减少重复维护。
