import { openAffair } from './affairs.js?v=0.17.0';
import { initializeStrategy } from './strategy.js?v=0.17.0';
import { initializeDoctrine } from './doctrines.js?v=0.17.0';
import { CORPS, corpsRank } from './development.js?v=0.17.0';
import { ARMS } from './martial.js?v=0.17.0';
import { hireHelper, helperBonus, validateHelpers } from './helpers.js?v=0.17.0';
import { finishDuty, developmentAction, raidBonus, RAID_INTEL, searchEquipment } from './camp-development.js?v=0.17.0';
import { hasOwn, requireRule, journal, count } from './utils.js?v=0.17.0';
import { ownHero } from './hero.js?v=0.17.0';
import { startBattle } from './battle.js?v=0.17.0';
import { grant } from './item.js?v=0.17.0';
...
export function attachTroops(s,n){
  ...
  initializeDoctrine(b);initializeStrategy(s,b);
...
export function validateCamp(s,check){
  ...
  for(const k of ['day','wood','food','troops','wounded','work','sorties'])check(num(c[k]),'寨务资源');check(c.troops+c.wounded<=c.buildings.barracks*20,'兵额');check(['solo','army'].includes(c.mode)&&hasOwn(TACTICS,c.tactic)&&num(c.deployment,100)&&c.deployment>=1,'军令');
  ...
  if(s.battle?.expedition){const e=s.battle.expedition;if(e.arm!==undefined)check(['infantry','ranged','cavalry'].includes(e.arm),'出征兵种');check(num(e.troops,100)&&e.troops<=c.troops&&hasOwn(TACTICS,e.tactic)&&!s.battle.guest,'出征兵力');}
}
