...
import { equipmentLoot } from './camp-development-ui.js?v=0.31.0';
import { skillLevel, unlockReason, skillUpgradeQuote, mountQuote, trainedSkill } from './growth.js?v=0.31.0';
...
export function stableMounts(s,d,esc,btn){
  ...
  return `<section class="location"><h2 class="subhead">副本寻骑 · 凭契入厩</h2><p>先通关对应历练副本，收取战果中的坐骑契，再到好汉详情安置专属脚力。已有坐骑保留；驯骑凭记只用于升阶，不是坐骑契。</p>
...
export function growthSources(s,d,esc,btn){
  ...
  return `<details class="fold-section" data-fold="growth-sources"><summary>养成材料去哪里找 · 切磋 ${drill}/3</summary>
    ...
    <p class="note">切磋：可直接在好汉页进行，耗 10 体力，每日 3 次；每次得残页 2、草料 1，第三次另得凭记 1。</p>
    ${btn('切磋研习 · 10 体力',{type:'martialDrill'},'secondary',drill>=3||s.player.stamina<10||!Object.values(s.heroes).some(h=>h.status==='owned'))}
    ...
    <p class="note">坐骑另需通关对应副本掉落的专属坐骑契，再在好汉详情领骑。切磋、每日差事和商店不会产出坐骑契；驯骑凭记仅用于坐骑升阶。</p>
