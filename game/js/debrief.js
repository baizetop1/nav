...
export function saveDebrief(s,b,wounded){s.lastBattle={at:s.clock,context:{...b.context},outcome:b.outcome,elapsed:b.elapsed||0,troops:b.expedition?.troops||0,wounded:Math.max(0,(s.camp?.wounded||0)-wounded),fallen:b.expedition?.fallen||0,team:b.team.map(({id,hp,maxHp})=>({id,hp,maxHp})),...(b.metrics?{metrics:b.metrics}:{})};}
export function validateDebrief(s,d,check){
 ...
 check(obj(r)&&num(r.at,s.clock)&&['victory','defeat','retreat'].includes(r.outcome)&&num(r.elapsed,180000)&&num(r.troops,1000)&&num(r.wounded,r.troops),'最近战果');if(r.fallen!==undefined)check(num(r.fallen,r.troops)&&r.fallen+r.wounded<=r.troops,'阵亡统计');check(obj(r.context)&&(r.context.type==='elite'?validEliteContext(r.context):['story','camp','frontier','dungeon','event','rotation','realm'].includes(r.context.type))&&typeof r.context.id==='string'&&/^[a-z][a-z0-9_]*$/.test(r.context.id),'复盘来源');check(Array.isArray(r.team)&&r.team.length>=1&&r.team.length<=3&&new Set(r.team.map(u=>u.id)).size===r.team.length,'复盘队伍');for(const u of r.team)check(d.by.heroes[u.id]&&num(u.maxHp,1000000)&&u.maxHp>0&&num(u.hp,u.maxHp),'复盘气血');if(r.metrics!==undefined)metrics(r.metrics,r.team.map(u=>u.id));
}
