import { gameSnapshot, slotId, slotName } from './portable.js?v=0.9.0';
...
export class CloudClient {
  ...
  async upload(id,revision,state,name){if(state.campaign||state.battle?.martial||state.camp?.arm){const version=await this.request('/v1/game-version');if(version.rotations!==1)throw new Error('请先更新云存档服务至 0.9.0，再上传轮换历练与兵种进度。本机进度已保留。');}id=slotId(id);return this.request(`/v1/slots/${id}`,{id,method:'PUT',revision,body:{state:gameSnapshot(state,this.data),name}});}
