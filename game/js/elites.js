...
import { attachTroops } from './camp.js?v=0.20.0';
import { grant } from './item.js?v=0.20.0';
import { requireRule, count, journal, hasOwn } from './utils.js?v=0.20.0';
export const ELITES={
...
export function elitePlan(s,id,kind='normal'){
 const model=hasOwn(ELITES,id)?ELITES[id]:null,hard=kind==='hard',used=s.daily.counters['elite_'+id]||0,n=s.camp?.mode==='army'?Math.min(s.camp.troops,s.camp.deployment):0,food=10+Math.ceil(n/2),stamina=hard?25:20;
