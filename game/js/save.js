import {validateRoles} from './hero-roles.js?v=0.44.0';
import {validateLessons} from './lessons.js?v=0.44.0';
import {validLessonContext} from './lessons-data.js?v=0.44.0';
import { validateRecruitSupport } from './recruit-support.js?v=0.44.0';
import {validateExpansion} from './expansion-save.js?v=0.44.0';
import { validateRealm, validRealmContext } from './realm.js?v=0.44.0';
import { staminaCap } from './logistics.js?v=0.44.0';
import { migrateRoster, rosterVersion } from './roster.js?v=0.44.0';
import { validateAlliances } from './volume-four-data.js?v=0.44.0';
import { validEliteContext } from './elites.js?v=0.44.0';
import { validateDebrief } from './debrief.js?v=0.44.0';
import { validateAffairs } from './affairs.js?v=0.44.0';
import { validateStrategy } from './strategy.js?v=0.44.0';
import { validateCommands } from './commands.js?v=0.44.0';
import { validateFrontier, POSTS } from './frontier.js?v=0.44.0';
import { validateDevelopment } from './development.js?v=0.44.0';
import { validateCampaign, validRotationContext } from './rotations.js?v=0.44.0';
import { validateQualities } from './quality.js?v=0.44.0';
import { newGame } from './core.js?v=0.44.0';
import { hasOwn, idPattern, requireRule } from './utils.js?v=0.44.0';
import { migrateBattle, BATTLE_LIMIT_MS } from './battle.js?v=0.44.0';
import { validateGrowth } from './growth.js?v=0.44.0';
import { validateGrowthBattle } from './growth-save.js?v=0.44.0';
import { validateCamp, RAIDS } from './camp.js?v=0.44.0';
...
export function validateSave(s,data) {
  ...
  validateExpansion(s,data,check);validateRealm(s,data,check);validateGrowth(s,data,check);validateCamp(s,check);validateCampaign(s,check);validateQualities(s,check);validateDevelopment(s,data,check);validateFrontier(s,data,check);
  ...
  validateRecruitSupport(s,data,check);
  validateRoles(s,check);validateLessons(s,check);validateCommands(s,check);validateStrategy(s,check);validateAffairs(s,check);
  const context=c=>check(c&&(c.type==='lesson'?validLessonContext(c):c.type==='realm'?validRealmContext(c):c.type==='elite'?validEliteContext(c):c.type==='frontier'?!!POSTS[c.id]:c.type==='rotation'?validRotationContext(c):c.type==='dungeon'?!!data.by.dungeons[c.id]:c.type==='camp'?!!s.camp&&hasOwn(RAIDS,c.id):c.type==='event'?!!data.by.events[c.id]:c.type==='story'&&(c.id==='huangni'||!!data.by.stories[c.id])),'交战来源');
