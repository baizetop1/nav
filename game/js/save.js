...
import { idPattern, requireRule } from './utils.js';
...
export function validateSave(s,data) {
  requireRule(object(s)&&s.version===1,'不支持这个存档版本。');safeTree(s);
  ...
  for(const h of data.heroes){const v=s.heroes[h.id];check(v&&['unknown','heard','known','available','owned'].includes(v.status)&&integer(v.level,1,40)&&integer(v.exp),'好汉状态');}
  ...
  for(const e of s.equipment){check(/^eq_\d+$/.test(e.uid)&&Number(e.uid.slice(3))<s.nextEquipment&&Object.hasOwn(data.by.equipments,e.item)&&integer(e.plus,0,5)&&(e.hero===null||s.heroes[e.hero]?.status==='owned'),'装备实例');if(e.hero){const slot=e.hero+':'+data.by.equipments[e.item].type;check(!slots.has(slot),'同一位置穿戴多件装备');slots.add(slot);}}
  ...
  check(Array.isArray(s.progress.visited)&&s.progress.visited.length<=30&&new Set(s.progress.visited).size===s.progress.visited.length&&s.progress.visited.every(id=>Object.hasOwn(data.by.maps,id)),'地图访问');
  ...
  check(Array.isArray(s.daily.events)&&s.daily.events.length<=20&&new Set(s.daily.events).size===s.daily.events.length&&s.daily.events.every(id=>data.by.events[id]),'际遇记录');
  ...
  if(s.battle){const b=s.battle;check(integer(b.round,1,100)&&Array.isArray(b.team)&&b.team.length>0&&b.team.length<=3&&Array.isArray(b.enemy)&&b.enemy.length>0&&b.enemy.length<=5&&[null,'victory','defeat','retreat'].includes(b.outcome)&&typeof b.guest==='boolean','战局');context(b.context);logs(b.log);
    for(const [side,units] of [['team',b.team],['enemy',b.enemy]])for(const u of units){check(u.side===side&&typeof u.name==='string'&&u.name.length<80&&typeof u.id==='string','战斗单位');check(side==='team'?!!data.by.heroes[u.id]:!!data.by.enemies[u.model],'战斗单位引用');for(const key of ['hp','maxHp','attack','defense','speed','strategy','rage'])check(integer(u[key],0,key==='rage'?100:1000000),'战斗属性');check(u.maxHp>0&&u.hp<=u.maxHp,'战斗气血');check(Array.isArray(u.skills)&&u.skills.length<=5&&u.skills.every(id=>data.by.skills[id]),'战斗技能');check(Array.isArray(u.statuses)&&u.statuses.length<=100&&u.statuses.every(e=>['poison','bleeding','stun','armor_break','rage'].includes(e.id)&&integer(e.turns,1,5)&&typeof e.value==='number'&&Number.isFinite(e.value)&&e.value>=0&&e.value<=1000),'战斗状态');}
...
export function parseSave(raw,data,now=Date.now()) {
  ...
  if(s.version===0){const base=newGame(data,now,s.rng||1);s={...base,...s,version:1,revision:s.revision||0,player:{...base.player,...s.player},heroes:{...base.heroes,...s.heroes},progress:{...base.progress,...s.progress}};
    if(Array.isArray(s.inventory))s.inventory=Object.fromEntries(s.inventory.map(i=>[i.id,i.count]));}
  return validateSave(s,data);
}
